...
- 应用启动时会自动拉起本地服务，无需手动部署
- 支持一键检查更新和自动安装
- 数据存储在系统用户目录中，不受浏览器清理影响
- 调试日志位于 \`%APPDATA%\\Author\\author-debug.log\`

### 重要提醒
- 所有数据默认存储在 **本地**，不会上传到任何官方云端
- 浏览器环境下若清除站点数据，未导出的内容可能丢失（桌面客户端不受影响）
- **API Key** 与偏好设置保存在本地持久化存储中
- 建议定期使用 💾 存档功能备份重要作品
...
- 本地化版本项目主页：[github.com/mleafish/author](https://github.com/mleafish/author)
- 原作者项目主页：[github.com/YuanShiJiLoong/author](https://github.com/YuanShiJiLoong/author)
...
当前本地化版本默认不提供云同步，所有数据都保存在你的当前设备中。
...
> 💡 如果你需要长期保留数据，建议定期导出存档文件。

### 数据冲突提示
本地版本不会自动合并不同设备上的数据。跨设备使用时，请以最新导出的存档文件为准。
    `,
    },
    {
        id: 'embedding',
